...
class Users:
    ...
    def __init__(self, token: str, org_id: Union[int, str], base_url: str = CODEGEN_BASE_API_URL):
    ...
    def get(self, user_id: Union[int, str]) -> User:
        ...
        """
        try:
            response = self.users_api.get_user_v1_organizations_org_id_users_user_id_get(
                org_id=self.org_id,
                user_id=str(user_id),
                authorization=f"Bearer {self.token}"
            )
    ...
    def get_page(self, page: int = 1, page_size: int = 50) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                'items': [User(user) for user in response.items],
