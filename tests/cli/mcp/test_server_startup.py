...
class TestMCPServerStartup:
    ...
    def test_server_startup_stdio(self):
        ...
        process = subprocess.Popen(
            [
                str(venv_python), "-c",
                "from codegen.cli.cli import main; main(['mcp', '--transport', 'stdio'])"
            ],
            env=env,
    ...
    def test_server_startup_invalid_transport(self):
        ...
        process = subprocess.Popen(
            [
                str(venv_python), "-c",
                "from codegen.cli.cli import main; main(['mcp', '--transport', 'invalid'])"
            ],
            env=env,
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
    ...
    def test_server_help_contains_expected_info(self):
        ...
        process = subprocess.Popen(
            [
                str(venv_python), "-c",
                "from codegen.cli.cli import main; main(['mcp', '--help'])"
            ],
            env=env,
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
