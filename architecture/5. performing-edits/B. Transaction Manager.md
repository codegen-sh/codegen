...
2. **Conflict Resolution**: Detects and resolves conflicts between transactions
3. **Transaction Execution**: Applies transactions in the correct order
4. **Resource Management**: Enforces limits on transaction count and execution time
5. **Change Tracking**: Generates diffs for applied changes
...
2. Transaction type (following the priority order)
...
2. **Contained Transactions**: One transaction completely contained within another
3. **Adjacent Transactions**: Transactions affecting adjacent byte ranges
...
2. **Execution Time**: Optional time limit for transaction processing
...
2. Files are processed one by one
3. For each file, transactions are executed in order
4. Diffs are collected for each modified file
5. The queue is cleared after successful commit
