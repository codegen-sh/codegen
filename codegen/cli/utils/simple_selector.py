"""Simple terminal-based selector utility for Windows."""
...
def _get_char():
    """Get a single character from stdin with Windows fallback."""
    try:
        ...
        # Fallback for systems without msvcrt (Unix-like)
        try:
            import termios
            import tty
...
def simple_select(
    ...
    try:
        ...
        for i, option in enumerate(options):
            ...
            if i == selected:
                print(f"  > {display_text}")  # Simple arrow for selected
            else:
        ...
        while running:
            ...
            elif allow_cancel and (
                key.lower() == "q" or key == "\x1b"
            ):  # q or Esc - cancel
                return None
...
def simple_org_selector(
    organizations: list[dict],
    current_org_id: Optional[int] = None,
    title: str = "Select Organization",
) -> dict | None:
