...
from codegen.sdk.cli.auth.session import CliSession
from codegen.sdk.cli.commands.start.docker_container import DockerContainer
from codegen.sdk.cli.errors import ServerError
from codegen.sdk.cli.rich.codeblocks import format_command
from codegen.sdk.cli.rich.spinners import create_spinner
from codegen.sdk.runner.clients.docker_client import DockerClient
from codegen.sdk.runner.enums.warmup_state import WarmupState
...
def _get_docker_client(session: CliSession) -> DockerClient:
    ...
    if (container := DockerContainer.get(repo_name)) is None:
        msg = f"codegen.sdk.runner does not exist for {repo_name}. Please run 'codegen start' from {session.config.repository.path}."
        raise click.ClickException(msg)

    if not container.is_running():
        msg = f"codegen.sdk.runner for {repo_name} is not running. Please run 'codegen start' from {session.config.repository.path}."
        raise click.ClickException(msg)
