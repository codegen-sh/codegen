...
Codegen provides a hosted MCP server that you can connect to directly without running a local server. This is useful for cloud-based environments or when you prefer not to run a local server.
...
- Useful for web-based integrations and remote connections
- Available as both a local server (by specifying host and port) and a hosted service
- Remote HTTP endpoint: `https://mcp.codegen.com/mcp/`
- Requires authentication via Bearer token in the Authorization header
- Enables cloud-based environments to connect to Codegen services
