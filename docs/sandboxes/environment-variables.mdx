...
| --------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------- |
| `NVM_DIR`                         | Specifies the directory where Node Version Manager (NVM) is installed. The exact path is set during sandbox initialization.                 |
| `PATH`                            | The system's execution search path. It's augmented to include the `bin` directory of the currently active Node.js version (managed by NVM). |
| `NVM_BIN`                         | Points to the `bin` directory of the currently active Node.js version. This is also set dynamically by NVM.                                 |
| `NODE_VERSION`                    | Indicates the version of Node.js that is currently active in the sandbox, as managed by NVM.                                                |
