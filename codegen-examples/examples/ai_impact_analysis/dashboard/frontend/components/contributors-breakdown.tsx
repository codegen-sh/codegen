...
export function ContributorsBreakdown({ contributors }: ContributorsBreakdownProps) {
  // Take top 5 contributors for the chart
  const topContributors = contributors.slice(0, 5)
  const otherContributors = contributors.slice(5)
  const otherCount = otherContributors.reduce((sum, [_, count]) => sum + count, 0)
  ...
  const chartData = [
    ...
    otherContributors.length > 0 ? { name: "Others", fullName: "Other Contributors", count: otherCount } : null,
  ].filter(Boolean)
                ...
                {contributors.slice(0, 10).map(([name, count], index) => (
                  <div key={name} className="flex justify-between items-center text-sm">
                    <div className="flex items-center">
                      ...
                      />
                      <span className="truncate max-w-[150px]">{name.split(" ")[0]}</span>
                    </div>
                    <div>{count}</div>
                  </div>
                ))}
