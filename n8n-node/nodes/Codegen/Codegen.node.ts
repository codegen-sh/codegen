			...
			try {
				...
				if (resource === 'agent') {
					...
					if (operation === 'run') {
						...
						const repoFullName = this.getNodeParameter('repoFullName', i) as string;
						const runType = this.getNodeParameter('runType', i) as string;
						const includeSource = this.getNodeParameter('includeSource', i) as boolean;
						const templateContext = JSON.parse(this.getNodeParameter('templateContext', i) as string);
						...
						// Make API request
						const response = await axios.post(endpoints.run, requestData, { headers });
